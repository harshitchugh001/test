...
def send_email():
    try:
        ...
        for url in urls:
            email_body = email_body.replace(url, f'http://localhost:5000/custom-redirect/?original_url={url}&token={token_generate}')

        # Add the HTML image tag within a valid HTML structure
        tracking_pixel_url = f'https://7441-2401-4900-1c35-9d1-ab8-e845-dca5-ea06.ngrok-free.app/track-pixel/{token_generate}'
        email_body += f'<pre><img src="{tracking_pixel_url}" alt="ahdshh" width="1" height="1" /> </pre>'
        print(email_body)
